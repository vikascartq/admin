...
export default function Candidate() {
    ...
    const { isOpen, onOpen, onOpenChange, formik, handleDelete, candidateList } = useContext(CandidateContext);
    return (
        <>
            <div className="p-4">
                        ...
                        <tbody>
                            {
                                candidateList.map((item, i) => (
                                    <tr className="border-t" key={`job-${i + 1}`}>
                                        ...
                                        <td className="px-4 py-2">{item?.skills.toString() || "-"}</td>
                                        <td className="px-4 py-2">
                                            <div className="flex items-center justify-end gap-x-4">
                                                <button onClick={() => {
                                                    ...
                                                    formik.setFieldValue("skills", item?.skills);
                                                    formik.setFieldValue("imageName", item?.imageName);
                                                    onOpen();
                                                }}><EditRowIcon /></button>
                                                <button onClick={() => {
                                                    formik.setFieldValue("id", item?._id);
